...
class XiaomiSm6225UdfpsHandler : public UdfpsHandler {
  ...
  private:
    ...
    void fodPressMonitorThread() {
        ...
        while (isRunning.load()) {
            // WATCHDOG: poll ngắn (200ms) thay vì 1000ms để có thể phát hiện
            // và tắt đèn gần như ngay lập tức khi trạng thái bị treo, thay
            // vì phải chờ đủ WATCHDOG_TIMEOUT_MS.
            int rc = poll(&fodPressStatusPoll, 1, 200);
